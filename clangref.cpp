LLVM Logo
indexnext |previous |LLVM Home | Documentation»Getting Started/Tutorials »LLVM Tutorial: Table of Contents »My First Language Frontend with LLVM Tutorial »3. Kaleidoscope: Code generation to LLVM IR
Documentation
...
Value *BinaryExprAST::codegen() {
  ...
  case '<':
    L = Builder->CreateFCmpULT(L, R, "cmptmp");
    // Convert bool 0/1 to double 0.0 or 1.0
    return Builder->CreateUIToFP(L, llvm::Type::getDoubleTy(*ctx->TheContext), "booltmp");
  default:
    return LogErrorV("invalid binary operator");
  }
}
